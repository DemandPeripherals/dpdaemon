 ...
 *  Name: stepb.c
 *
 *  Description: Driver for the bipolar stepper motor peripheral
 *
 *  Resources:
 ...
 *  - Limits and defines
 **************************************************************/
        // STEPB register definitions
#define STEPB_REG_COUNT    0x00
#define STEPB_REG_ADD      0x02
#define STEPB_REG_CNFG     0x04
#define STEPB_REG_PERIOD   0x05
#define STEPB_REG_HOLD     0x06
#define MODE_OFF          'o'
#define MODE_FULL         'f'
#define MODE_HALF         'h'
 ...
 *  - Data structures
 **************************************************************/
    // All state info for an instance of an stepb
typedef struct
{
...
static void noAck(void *, STEPBDEV *);
static void sendconfigtofpga(STEPBDEV *, int *plen, char *buf);
...
{
    STEPBDEV *pctx;    // our local device context

    // Allocate memory for this peripheral
    pctx = (STEPBDEV *) malloc(sizeof(STEPBDEV));
    if (pctx == (STEPBDEV *) 0) {
        // Malloc failure this early?
        edlog("memory allocation failure in stepb initialization");
        return (-1);
    }

    // Init our STEPBDEV structure
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    pctx = (STEPBDEV *)(pslot->priv);  // Our "private" data is a STEPBDEV
    ...
    if ((pkt->reg != STEPB_REG_COUNT) || (pkt->count != 2) ||
        ((pkt->cmd & DP_CMD_AUTO_MASK) == DP_CMD_AUTO_DATA)) {
        edlog("invalid stepb packet from board to host");
        return;
    }
    ...
    char    *buf)
{
    ...
    if ((cmd == EDGET) && (rscid == RSC_COUNT)) {
        ...
        pkt.reg = STEPB_REG_COUNT;
        pkt.count = 2;
        // send the packet.  Report any errors
        txret = dpi_tx_pkt(pmycore, &pkt, 4); // 4 header
        if (txret != 0) {
...
static void sendconfigtofpga(
    STEPBDEV *pctx,    // This peripheral's context
    int     *plen,     // size of buf on input, #char in buf on output
    char    *buf)      // where to store user visible error messages
{
    DP_PKT   pkt;      // send write and read cmds to the stepb
...
static void noAck(
    ...
    STEPBDEV *pctx)    // the peripheral with a timeout
{
    // Log the missing ack
    edlog(E_NOACK);
