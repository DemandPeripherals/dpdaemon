 ...
 *  Name: out4.c
 *
 *  Description: Device driver for the out4 peripheral
 *
 *  Hardware Registers:
 ...
 *  - Data structures
 **************************************************************/
    // All state info for an instance of an out4
typedef struct
{
...
static void out4user(int, int, char*, SLOT*, int, int*, char*);
static void noAck(void *, OUT4DEV *);
static int  out4tofpga(OUT4DEV *);
    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    pctx->outval = 0xf;        // Matches Verilog default value
    ...
    pslot->rsc[RSC_OUTVAL].pgscb = out4user;
...
/**************************************************************
 * out4user():  - The user is reading or writing to the output.
 * Get the value and update the out4 on the BaseBoard or read the
    ...
    char    *buf)
{
    ...
    int      newout4;  // new value to assign the out4
    ...
    ret = sscanf(val, "%x", &newout4);
    if ((ret != 1) || (newout4 < 0) || (newout4 > 0xf)) {
    ...
    txret =  out4tofpga(pctx);   // This peripheral's context
    if (txret != 0) {
...
/**************************************************************
 * out4tofpga():  - Send outval to the FPGA card.  Return
 * zero on success
 **************************************************************/
int out4tofpga(
